...
fun main(firstProduct: String) {
    ...
    if (result != null) {
        ...
        for ((index, product) in products.withIndex()) println("Финальная реакция ${index + 1}: $product")
    } else println("Реакция не обнаружена")
}
...
private fun getReactionResult(product1: String): String? {
    ...
    for (reactions in allReactions) for (reaction in reactions) if (reaction[0] == product1) return reaction[1]
    return null
}