...
class Parser {
    ...
    parseEquation() {
        ...
        while (true) {
            ...
            if (next === "+") {
                ...
                throw new ParseError("Ожидался плюс или знак равенства");
            }
        }
        ...
        while (true) {
            ...
            if (next === null) break;
                ...
                throw new ParseError("Ожидался плюс или конец");
            }
        }
    ...
    parseTerm() {
        ...
        while (true) {
            ...
            else if (next !== null && /^[0-9]+$/.test(next)) throw new ParseError("Недопустимый терм — число не ожидалось");
            else break;
        }
        let charge = null;
        if (next === "^") {
            this.tok.consume(next);
            next = this.tok.peek();
            if (next === null) throw new ParseError("Ожидалось число или знак");
            else {
            ...
            else throw new ParseError("Ожидался знак");
            this.tok.take();
        }
        if (electron) {
            if (items.length > 0) throw new ParseError("Недопустимый терм — электрон должен быть отдельно");
            if (charge === null) charge = -1;
            if (charge !== -1) throw new ParseError("Недопустимый терм — недопустимый заряд для электрона");
        } else {
            if (items.length === 0) throw new ParseError("Недопустимый терм — пустой");
    ...
    parseGroup() {
        ...
        while (true) {
            ...
            if (next === "(") items.push(this.parseGroup());
                ...
                this.tok.consume(next);
                if (items.length === 0) throw new ParseError("Пустая группа");
                break;
            } else {
                throw new ParseError("Ожидался элемент, группа или закрывающая скобка");
            }
        }
...
class Tokenizer {
    ...
    peek() {
        ...
        if (match === null) throw new ParseError("Недопустимый символ");
        return match[0];
    }
